...
GColor enamel_get_screen_color(){
	Tuple* tuple = dict_find(&s_dict, 1535730288);
...
GColor enamel_get_clock_bg_color(){
	Tuple* tuple = dict_find(&s_dict, 3475840930);
	return tuple ? GColorFromHEX(tuple->value->int32) : GColorFromHEX(0x000000);
}
// -----------------------------------------------------
...
GColor enamel_get_clock_fg_color(){
	Tuple* tuple = dict_find(&s_dict, 3302461566);
...
GColor enamel_get_hour_hand_color(){
	Tuple* tuple = dict_find(&s_dict, 308086781);
	return tuple ? GColorFromHEX(tuple->value->int32) : GColorFromHEX(0xFFFFFF);
}
// -----------------------------------------------------
...
GColor enamel_get_minute_hand_color(){
	Tuple* tuple = dict_find(&s_dict, 636154801);
	return tuple ? GColorFromHEX(tuple->value->int32) : GColorFromHEX(0xFFFFFF);
}
// -----------------------------------------------------
...
GColor enamel_get_subdial_highlight_color(){
	Tuple* tuple = dict_find(&s_dict, 2532035718);
	return tuple ? GColorFromHEX(tuple->value->int32) : GColorFromHEX(0xFFFFFF);
}
// -----------------------------------------------------
...
bool enamel_get_animate_minutes(){
	Tuple* tuple = dict_find(&s_dict, 2440279532);
...
bool enamel_get_display_seconds(){
	Tuple* tuple = dict_find(&s_dict, 525570439);
	return tuple ? tuple->value->int32 == 1 : true;
}
// -----------------------------------------------------
...
static uint32_t prv_map_messagekey(const uint32_t key){
	if( key == MESSAGE_KEY_screen_color) return 1535730288;
	if( key == MESSAGE_KEY_clock_bg_color) return 3475840930;
	if( key == MESSAGE_KEY_clock_fg_color) return 3302461566;
	if( key == MESSAGE_KEY_hour_hand_color) return 308086781;
	if( key == MESSAGE_KEY_minute_hand_color) return 636154801;
	if( key == MESSAGE_KEY_subdial_highlight_color) return 2532035718;
	if( key == MESSAGE_KEY_animate_minutes) return 2440279532;
	if( key == MESSAGE_KEY_display_seconds) return 525570439;
	return 0;
}
...
static bool prv_is_setting_message(const DictionaryIterator *iter){
	if( dict_find(iter, MESSAGE_KEY_screen_color) ) return true;
	if( dict_find(iter, MESSAGE_KEY_clock_bg_color) ) return true;
	if( dict_find(iter, MESSAGE_KEY_clock_fg_color) ) return true;
	if( dict_find(iter, MESSAGE_KEY_hour_hand_color) ) return true;
	if( dict_find(iter, MESSAGE_KEY_minute_hand_color) ) return true;
	if( dict_find(iter, MESSAGE_KEY_subdial_highlight_color) ) return true;
	if( dict_find(iter, MESSAGE_KEY_animate_minutes) ) return true;
	if( dict_find(iter, MESSAGE_KEY_display_seconds) ) return true;
	return false;	
}
